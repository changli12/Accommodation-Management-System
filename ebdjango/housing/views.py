...
def amenity_admin_delete(request):
    ...
    cursor = connection.cursor()
    cursor.execute(
        "DELETE FROM `housing_amenity` WHERE (`amenity_id` = %s)",
        [amenity_id])
    amenity_info =amenity.objects.raw("SELECT * FROM housing_amenity")
    return render(request, 'amenity_admin.html', {'amenity_info': amenity_info})
...
def payment_admin_add(request):
    ...
    if (pay_record=="Charge"):
        ...
        pay_record = 1
    cursor = connection.cursor()
    cursor.execute('INSERT INTO `housing_payment` ( `fee`, `pay_date`, `payment_type`, `pay_record`, `renter_id`, `room_id`) VALUES ( %s, %s, %s, %s, %s, %s)',[fee, pay_date, payment_type, pay_record,renter_id,room_id])
...
def housing_renter_add(request):
    ...
    cursor = connection.cursor()
    cursor.execute('INSERT INTO `housing_renter` ( `renter_id`, `email`, `firstname`, `lastname`, `gender`, `renter_pwd`, `phone`, `carplate`, `balance`, `birthday`,  `country`) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)',[renter_id, renter_email, renter_firstname, renter_lastname, renter_gender, renter_pwd, renter_phone, renter_carplate,renter_balance, renter_birthday, renter_country])
    cursor.execute('INSERT INTO `housing_lease` ( `rent_fee`, `movein_date`, `moveout_date`, `renter_id`,`room_id`) VALUES ( %s, %s, %s, %s, %s)',[rent_fee, movein_date, moveout_date, renter_id, room_number])
    cursor.execute(
